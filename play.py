...
from atari_env import AtariTennisWrapper
...
def play(env, transpose=True, fps=30, zoom=None, callback=None, keys_to_action=None):
    ...
    while running:
        ...
        clock.tick(fps)
        count += 1
        if count % img_per_frame == 0:
            # Save image per 10 frames
            env.env.ale.saveScreenPNG('./tennis_img/tennis' + str(num_img) + '.png')
            num_img += 1
    pygame.quit()
...
def main():
    ...
    if args.env == 'TennisNoFrameskip-v4':
        env = AtariTennisWrapper(gym.make(args.env))
